	...
	var Ajax = function () {
	  ...
	  _createClass(Ajax, [{
	    ...
	    value: function request() {
	      ...
	      var params = options.params || options.data;
	    ...
	    key: 'loadJsonp',
	    value: function loadJsonp(url, onSuccess, onError) {
	      var timeout = arguments.length <= 3 || arguments[3] === undefined ? 0 : arguments[3];


	      var stamp = [new Date().getTime(), Math.floor(Math.random() * 500)].join('_');
	      var cbHandler = 'ajax_cb_' + stamp;
	    ...
	    value: function fetch(url, data, callback, type) {
	      if (_common2.default.isFunc(data)) {
	        type = type || callback;
	        callback = data;
	        data = undefined;
	      }
	    ...
	    value: function post(url, data, callback, type) {
	      if (_common2.default.isFunction(data)) {
	        type = type || callback;
	        callback = data;
	        data = undefined;
	      }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Device = function () {
	  function Device() {
	    ...
	    this.constructor.sdkVersion = "2.0.3";
	  ...
	  _createClass(Device, [{
	    ...
	    value: function getMWMetaData(userAgent) {
	      ...
	      metaArr.forEach(function (value, i) {
	        var kv = value.split('/');
	        meta[kv[0]] = kv[1] || '';
	      });
	      return meta;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _bannerHelper = __webpack_require__(14);
	...
	var Render = function () {
	  ...
	  _createClass(Render, [{
	    ...
	    key: 'parseTemplate',
	    value: function parseTemplate(data) {
	      ...
	      if (_common2.default.isObject(template)) {
	        template = template[banner.key] || defaultTemplate;
	      }

	      // 如果是自定义渲染方法,则执行开发者自定义的渲染方法
	      if (_common2.default.isFunc(template)) {
	      ...
	      if (!mwBlock.getAttribute(RENDERED)) {
	        mwBlock.addEventListener('click', function () {
	          ...
	          if (Number(data.dt) === 4) {
	            ...
	            _this2.openMwBlockDialog(mwBlock, url);
	            //window.location = url;
	          }
	        });
	      }
	...
	var Mlink = function () {
	  ...
	  _createClass(Mlink, [{
	    ...
	    value: function getRealUrl(url, params, defaultParams) {
	      ...
	      params = _common2.default.mix(defaultParams, params);
	    ...
	    key: 'showWeixinDownloadModal',
	    value: function showWeixinDownloadModal() {
	    ...
	    key: 'loadDefaultParams',
	    value: function loadDefaultParams() {

	      return new _promise2.default(function (resolve, reject) {

	        markting.load().then(function (response) {
	    ...
	    key: 'getDeferrerInfo',
	    value: function getDeferrerInfo() {
...
/***/ function(module, exports) {
	...
	var BannerHelper = function () {
	  function BannerHelper(bannerData) {
	    _classCallCheck(this, BannerHelper);
